...
// public class PythonEnumWriter implements PythonEnumGenerator::EnumWriter
public class PythonEnumWriter
{
    PythonEnumGenerator parentClass = null;
    Properties aliases = null;
    Set<String> enumNames = new HashSet<>();

    private PythonEnumWriter()
    {
    } 

    public PythonEnumWriter(PythonEnumGenerator aParent)
    {
        parentClass = aParent;
    }
    ...
    private void writeEnumDefinitions(StringBuilder sb, GeneratedEnumClasses.EnumElem enumElement, String className)
    {
        ...
        for (GeneratedEnumClasses.EnumRowElem row : enumElement.elems) {
            ...
            if(aliases != null && aliases.getProperty(row.value)!=null) {
                ...
                enumName = StringUtils.createEnumName(enumNames, StringUtils.normalizeDescription(row.description), false);                
            }
            writeOneEnum(sb, row, enumName, firstOne);
            if (firstOne == true) firstEnumName = enumName;
            firstOne = false;
    ...
    private void writeOneEnum(StringBuilder sb, GeneratedEnumClasses.EnumRowElem row, String enumName, boolean isFirstOne)
    {
        String xrefName = null;
        String enumStatement = "";

        int enumValue = Integer.parseInt(row.value);
        ...
        if (parentClass.pythonEnumAliases !=null && parentClass.pythonEnumAliases.getProperty(enumName)!= null) {
            enumName = parentClass.pythonEnumAliases.getProperty(enumName);
        }
        ...
        // if (isFirstOne == true)
        // {
        //     enumName = "default = " + enumName;
        // }
        
        if (xrefName == null)
        {
            enumStatement = String.format(parentClass.disenumpart2Template, enumName, enumValue, enumDescription);
        }
        else
        {
            sb.append(String.format(parentClass.disenumcommentTemplate, row.xrefclassuid, xrefName));
            enumStatement = String.format(parentClass.disenumpart21Template, enumName, enumValue, enumDescription);            
        }

        // if (isFirstOne)
        // {
        //     enumStatement = StringUtils.tabs(1) + "default = " + enumStatement.trimStart();
        // }
    ...
    private void writeEnumDescriptions(StringBuilder sb, GeneratedEnumClasses.EnumElem enumElement, String className)
    {
        ...
        String mapName = String.format("_%s_descriptions", className);
        ...
        for (GeneratedEnumClasses.EnumRowElem row : enumElement.elems) {
            ...
            enumStatement = String.format("%s%s.%s : \"%s\",", StringUtils.tabs(3), className, enumName, enumDescription);  
        ...
        sb.append("\n");
        sb.append(String.format(parentClass.disenumtostringTemplate, mapName, mapName, className));
    ...
    private void writeMarshalOperators(StringBuilder sb, GeneratedEnumClasses.EnumElem enumElement, String className)
    {
        ...
        String enumSizeReader = "unsigned_byte";
        if (enumSize == 16) enumSizeReader = "unsigned_short";
        else if (enumSize == 32) enumSizeReader = "unsigned_int";

        sb.append(String.format(parentClass.disenummarshalersTemplate, enumSize));
    ...
    private void writeOutFile(StringBuilder sb, String className)
    {
        ...
        File targetFile = new File(parentClass.outputDirectory, pythonFileName + ".py");
